...
from src.for_test.getllm import return_llm_obj
from src.class_finbot import FinBotClass, create_dynamic_data_dict
...
from src.for_test.envnew import (PG_HOST_NAME, PG_DATABASE_NAME, PG_DATABASE_UNAME, PG_DATABASE_PWD,
                        SQL_SCHEMA_NAME,SQL_DWH_TBL)
...
def preload_data():
    ...
    with refresh_lock:
        try:
            ...
            df_loaded = pd.read_csv("Data/heartbeat_6_months.csv")

            # Dynamically create data dictionary after loading the DataFrame
...
async def query_handler(request: QueryRequest):
    ...
    try:
        ...
        print("chat engine response:", chatbot_engine_response)
        ...
        print("after formatting:", payload)
...
async def start_scheduler_only():
    ...
    #     id=DAILY_JOB_ID,
    #     replace_existing=True,
    #     misfirae_grace_time=None  # Run if within 1 hour late
    # )
    # scheduler.start()
...
#uvicorn app.main_local:app --reload
